...
const generateData = (pr) => {
 return  randomFinancial()
            .startDate(new Date())
            .startPrice(pr)(30).map((itm, i) =>{
...
const chartView = {
   ...
   ii: 0,
   priceSeed: null,  
   data: [{type: "area", markerType: "none", fillOpacity: 0.3, lineColor: "white", color:"#00FF00",dataPoints: []}],
   render: function(){
...
const setCard = (cardId, price=88, symb) => {
   ...
   let thisSymb = symb;
   let pointStream = randomFinancial()
            .startDate(new Date())
            .startPrice(price).stream();
