...
class CallPut extends Component {
  constructor(props) {
    super(props);
    this.clock = null;
    this.state = {
    ...
    return (
      <div className="option-buy-sell reduct"> 
        ...
        <div id="Bid" className={this.state.bidOpen ? "bounce-in-fast" : "hide-elm"}>
          <p className="pos-type-header call-t">Call Position </p>
        ...
        <p className="text-wrap-values time-ticker time-v" onClick={this.timeMenuOpen.bind(this)}><canvas id={this.props.timerId} height={18} width={66}></canvas></p>

        <p className="text-wrap-values time-ticker"><strong>{this.state.qnty}</strong></p>
        <div className="text-wrap-values plus-minus sub-v" onClick={this.plusMinus.bind(this, -1)}><i className="material-icons">remove</i></div>

        <div className="text-wrap-values plus-minus add-v" onClick={this.plusMinus.bind(this, 1)}> <i className="material-icons">add</i></div>
